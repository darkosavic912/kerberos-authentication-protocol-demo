...
public class KerberosClient {
    ...
    public static void main(String[] args) {
        ...
        try (Socket client = new Socket(serverName, port);
            ...
            //1.1. Compute password hash (SHA-256)
            MessageDigest md = MessageDigest.getInstance("SHA-256");
            byte[] passwordHash = md.digest(passwordAlice.getBytes(StandardCharsets.UTF_8));

            //1.2. Regenerate client secret key (Ka) from password hash (first 16 bytes for AES-128)
            byte[] keyMaterial = Arrays.copyOfRange(passwordHash, 0, 16);
            SecretKey userKeyKa = new SecretKeySpec(keyMaterial, "AES");

            //1.3. Alice receives encrypted payload containing Session Key (Sa) and TGT
            byte[] saTgtEncrypted = Base64.getDecoder().decode(in.readUTF());

            //1.4. Decrypt SA+TGT payload using Ka (Note: AES_cipher decrypt handles internal IV extraction)
            // After this Ka is not used anymore
            byte[] saTgt = new AES_cipher().decrypt(userKeyKa, saTgtEncrypted);

            //1.5. Extract encrypted Session Key (Sa) and encrypted TGT from decrypted byte stream
            ...
            //1.6. Alice decrypt ciphertext and take session key
            ...
            //2.2. Receive response ticket TGT encrypted with Session Key (Sa)
            byte[] responseEncrypted = Base64.getDecoder().decode(response);

            //2.3. Decrypt with Session key Sa
            byte[] decryptedResponse = new AES_cipher().decrypt(saKey, responseEncrypted);

            //2.4. Extract form TGT  target file path and Service Session Key (Kab)
            ...
            //2.5. Read encrypted file from disk and decrypt data it using Kab
