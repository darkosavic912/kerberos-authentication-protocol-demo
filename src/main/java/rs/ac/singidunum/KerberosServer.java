        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            while (true) {
                ...
                try (Socket server = serverSocket.accept();
                    ...
                    //1.7. Encrypt TGT using KDC Master Key
                    byte[] encryptedTgt = new AES_cipher().encrypt(kdcMasterKey, tgtPayload);

                    //1.8. Merge [Session Key Sa] + [Encrypted TGT] into a single structured byte array
                    ...
                    //1.9. Encrypt combined payload with client secret key (Ka)/ (Sa + TGT)
                    byte[] encryptedSaTgtPayload = new AES_cipher().encrypt(userKeyKa, saTgtPayload);

                    //1.10. Send response (encrypted Sa + TGT) back to Client
                    ...
                    //2.1. Takes from packet TGT, file name and authenticator
                    ...
                    //2.6. Prepare target data file on server
                    ...
                    //2.9. Create service ticket containing [Target File Name] + [Kab Key]
                    ...
                    //2.10. Encrypt reply ticket with Session Key (Sa)
